...
public class LogUtils {
    ...
    private static boolean LOG_DEBUG = BuildConfig.LOG_MODE;
...
/*    public static void crash2File(Context context, Throwable throwable) {
    ...
    /**
     * 将HashMap中的参数 组装成post方法的字符串 一般用于网络获取是url的组装
     */
    public static String getParametersToString(String url, Map<String, Object> map) {
        if (map == null) {
            return "";
        }
        Iterator<String> enu = map.keySet().iterator();
        StringBuilder sb = new StringBuilder();
        sb.append(url);
        String item;
        try {
            while (enu.hasNext()) {
                item = enu.next();
                sb.append(item).append("=").append(URLEncoder.encode(map.get(item).toString(), "UTF-8")).append("&");
            }
